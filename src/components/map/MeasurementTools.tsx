...
export function MeasurementTools() {
  ...
  useEffect(() => {
    ...
    // Define handlers that capture the current activeTool
    const onDrawCreate = (e: DrawEvent) => {
    ...
    const onDrawUpdate = (e: DrawEvent) => {
      const feature = e.features[0];
      updateMeasurement(feature);
    };

    map.on('draw.create', onDrawCreate as any);
    map.on('draw.update', onDrawUpdate as any);

    return () => {
      map.removeControl(drawRef.current);
      map.off('draw.create', onDrawCreate as any);
      map.off('draw.update', onDrawUpdate as any);
    };
  }, [map, activeTool, addCircleMeasurement, addSquareMeasurement, addLineMeasurement, updateMeasurement]);
